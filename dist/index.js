...
import { posix as path3 } from "path";
...
`;
  ...
  async insert(insert) {
    if (!this._options.styles)
      return "";
    const template = this._options.styles.lang === "css" ? "" : await promisify(readFile)(
...
function BuildPlugin(iconsPattern, options) {
  ...
  return {
    ...
    transform(code) {
      ...
      full_path = path3.join(base, filename);
...
function VuePlugin(iconsPattern, options) {
  ...
  return {
    ...
    async load(id) {
      ...
      const { name, base: filename2 } = parse(path4);
